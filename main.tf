...
resource "aws_s3_bucket" "request_bucket" {
  bucket = "caleb-translate-requests"
  force_destroy = true
}
 
# Response bucket (output JSON)
resource "aws_s3_bucket" "response_bucket" {
  bucket = "caleb-translate-responses"
  force_destroy = true
}
 
# IAM Role for Lambda
resource "aws_iam_role" "lambda_role" {
  name = "caleb-translate-lambda-role"
...
resource "aws_lambda_function" "translate_lambda" {
  function_name = "caleb-translate-lambda"
